...
do
  ...
  case $key in
    ...
    --help|-h)
      ...
      echo "Options:"
      echo "  --version, -v       指定版本号。如果未指定，将使用 mdcx/consts.py 文件中的 LOCAL_VERSION"
...
getAppVersionFromConfig () {
  local configPath="mdcx/consts.py"
...
if [ -z "$APP_VERSION" ]; then
  log_info "APP_VERSION 未设置，尝试从 mdcx/consts.py 获取..."
  APP_VERSION=$(getAppVersionFromConfig)
  if [ -z "$APP_VERSION" ]; then
    log_error "❌ APP_VERSION 未设置且无法从 mdcx/consts.py 中找到!"
    exit 1
  else
