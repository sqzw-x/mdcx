...
do
  ...
  case $key in
    ...
    --help|-h)
      ...
      echo "Options:"
      echo "  --version, -v       指定版本号。如果未指定，将使用 mdcx/config/manual.py 文件中的 LOCAL_VERSION"
...
getAppVersionFromConfig () {
  local configPath="mdcx/config/manual.py"
...
if [ -z "$APP_VERSION" ]; then
  log_info "APP_VERSION 未设置，尝试从 mdcx/config/manual.py 获取..."
  APP_VERSION=$(getAppVersionFromConfig)
  if [ -z "$APP_VERSION" ]; then
    log_error "❌ APP_VERSION 未设置且无法从 mdcx/config/manual.py 中找到!"
    exit 1
  else
