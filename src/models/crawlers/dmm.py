...
from models.base.web_compat import get_text_, post_json_
...
def get_tv_jp_data(real_url):
    ...
    result, response = post_json_("https://api.tv.dmm.co.jp/graphql", headers=headers, json=data)
...
def get_tv_com_data(number):
    ...
    result, response = post_json_("https://api.tv.dmm.com/graphql", headers=headers, json=data)
    if result and response.get("data"):
...
def main(
    ...
    try:
        ...
        if "tv.dmm." not in real_url:
            result, htmlcode = get_text_(real_url, cookies=cookies)
            if not result:  # 请求失败
