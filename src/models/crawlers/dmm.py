...
def get_tv_com_data(number):
    ...
    result, response = post_html('https://api.tv.dmm.com/graphql', headers=headers, json=data, json_data=True,
                                 keep=False)
    if result and response.get('data'):
    ...
    try:
        ...
        if not real_url or 'tv.dmm.com' in real_url:
            if not real_url:
                ...
                elif number_00.lower().startswith('ugm') or number_00.lower().startswith(
                        'dmi') or number_00.lower().startswith('whm'):
