...
try:
    ...
    db.execute("select Name, Alias from Names where Alias=''")  # 必须实际执行查询才能判断是否连接成功
except Exception as e:
    signal.show_traceback_log(traceback.format_exc())
    signal.show_log_text(f" ❌ 数据库连接失败: {e}")
    db = None


def update_actor_info_from_db(actor_info: EMbyActressInfo) -> bool:
    if db is None:
        signal.show_log_text(f" ❌ 数据库连接失败")
        return False
